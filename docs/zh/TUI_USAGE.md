...
如果你想了解更完整的运行时控制流、Prompt/Skill 执行模型、MCP 语义、以及 Canvas（Git 图）如何被重建，请同时阅读 `docs/zh/RUNTIME_FLOW_AND_CANVAS.md`。
...
- `node bin/ds.js`：启动守护进程，并打开本地 Web + TUI。
- `node bin/ds.js --tui`：仅启动守护进程 + TUI。
- `node bin/ds.js --stop`：停止**守护进程本身**（不是仅停止某个 quest）。
